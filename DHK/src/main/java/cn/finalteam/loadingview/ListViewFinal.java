...
/**
 * Desction: Author:pengjianbo Date:16/3/7 上午11:34
 */
public class ListViewFinal extends ListView implements OnScrollBottomListener
{
	...
	private int mLoadMoreViewBottom, mLoadMoreViewTop, mLoadMoreViewLeft,
			mLoadMoreViewRight;
	...
	private void init(Context context, AttributeSet attrs)
	{
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.LoadingViewFinal);

		if (a.hasValue(R.styleable.LoadingViewFinal_loadMoreMode))
		{
			mLoadMoreMode = LoadMoreMode.mapIntToValue(
					a.getInt(R.styleable.LoadingViewFinal_loadMoreMode, 0x01));
		} else
		{
		...
		{
			mNoLoadMoreHideView = a.getBoolean(
					R.styleable.LoadingViewFinal_noLoadMoreHideView, false);
		} else
		{
		...
		if (a.hasValue(R.styleable.LoadingViewFinal_loadMoreView))
		{
			try
			{
				String loadMoreViewName = a
						.getString(R.styleable.LoadingViewFinal_loadMoreView);
				Class clazz = Class.forName(loadMoreViewName);
				Constructor c = clazz.getConstructor(Context.class);
				ILoadMoreView loadMoreView = (ILoadMoreView) c
						.newInstance(context);
	...
	public void hideLoadMoreView()
	{
		...
		mLoadMoreView.getFooterView().setPadding(0,
				-mLoadMoreView.getFooterView().getHeight(), 0, 0);
	}
	...
	private void showLoadMoreView()
	{
		...
		mLoadMoreView.getFooterView().setPadding(mLoadMoreViewLeft,
				mLoadMoreViewTop, mLoadMoreViewRight, mLoadMoreViewBottom);
	}
	...
	private DataSetObserver mDataObserver = new DataSetObserver()
	{
		...
		public void onChanged()
		{
			dispatchTouchEvent(MotionEvent.obtain(SystemClock.uptimeMillis(),
					SystemClock.uptimeMillis(), MotionEvent.ACTION_CANCEL, 0, 0,
					0));
		}
