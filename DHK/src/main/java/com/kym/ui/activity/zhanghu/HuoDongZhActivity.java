...
public class HuoDongZhActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void getdata() {
        ...
        Connect.getInstance().post(HuoDongZhActivity.this, IService.GET_ACCOUNT_BALANCE, null, new Connect.OnResponseListener() {
            @Override
            public void onSuccess(Object result) {
                AccountBalance response = (AccountBalance) JsonUtils.parse((String) result, AccountBalance.class);
                if (response.getResult().getCode() == 10000) {
                    int balance = response.getData().getTotalactivity_money();
                    int todayMoney = response.getData().getTodayactivityMoney();
                    int yesterdayMoney = response.getData().getYesterdayactivityMoney();
                    int total_money = response.getData().getTotalactivity_cash();
                    try {
    ...
    private void initview() {
        ...
        ImageView imageV_fanhui1 = findViewById(R.id.head_img_right);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.lay_yue_tx:
                Intent intent5 = new Intent(HuoDongZhActivity.this, HuoDongTiXianActivity.class);
