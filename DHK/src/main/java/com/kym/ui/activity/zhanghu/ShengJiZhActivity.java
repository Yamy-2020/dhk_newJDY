...
public class ShengJiZhActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void getdata() {
        ...
        Connect.getInstance().post(ShengJiZhActivity.this, IService.GET_ACCOUNT_BALANCE, null, new Connect.OnResponseListener() {
            @Override
            public void onSuccess(Object result) {
                AccountBalance response = (AccountBalance) JsonUtils.parse((String) result, AccountBalance.class);
                if (response.getResult().getCode() == 10000) {
                    int balance = response.getData().getTotalupgrade_money();
                    int todayMoney = response.getData().getTodayupgradeMoney();
                    int yesterdayMoney = response.getData().getYesterdayupgradeMoney();
                    int total_money = response.getData().getTotalupgrade_cash();
                    try {
    ...
    private void initview() {
       ...
       /* if (OMID.equals("VIB0T31Q2L7DNDK5")) {
            textV_title.setText("办卡分润");
        } else {*/
            textV_title.setText("升级账户");
//        }/**/
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.lay_yue_tx:
                Intent intent5 = new Intent(ShengJiZhActivity.this, ShengJiTiXianActivity.class);
