...
public class XinYongKaActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_xin_yong_ka);
        initview();
    }

    private void getdata() {
        loadDialogUti = new DialogUtil(this);
        Connect.getInstance().post(XinYongKaActivity.this, IService.GET_ACCOUNT_BALANCE, null, new Connect.OnResponseListener() {
            @Override
            public void onSuccess(Object result) {
                AccountBalance response = (AccountBalance) JsonUtils.parse((String) result, AccountBalance.class);
                if (response.getResult().getCode() == 10000) {
                    int balance = response.getData().getTotal_money();
                    int todayMoney = response.getData().getTodayMoney();
                    int yesterdayMoney = response.getData().getYesterdayMoney();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.head_img_right:
                ...
                intent.putExtra("type", 4);//数字代表 4还款账户,  5返现  6 收款  7 代偿   8 升级  9 活动
            ...
            case R.id.lay_yue_tx:
                Intent intent5 = new Intent(XinYongKaActivity.this, BankTiXianActivity.class);
