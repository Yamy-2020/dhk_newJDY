...
/**
 * Created by zhangph on 2018/1/24.
 */
...
public class CalendarView extends View {
     ...
     * 可选择的天数
     */
    private int chooseArea = 29;

    /**
     * 可选择的开始位置
     */
    private long startLillis = System.currentTimeMillis();
     ...
     * 不可选中文字颜色
     */
    private int mconnotSelectTextColor;
    /**
     * 默认文字大小
     */
     ...
     * 不可选中文字大小
     */
    private float mconnotSelectTextSize;
    /**
     * 默认天的背景
     */
     ...
     * 不可选天的背景
     */
    private Drawable mconnotSelectDayBackground;
    /**
     * 日期格式化格式
     */
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        int cannotselectTextColor = a.getColor(R.styleable.CalendarView_cv_cannotselectTextColor, Color.BLACK);
        setCannotSelectTextColor(cannotselectTextColor);
        ...
        float selectTextSizes = a.getDimension(R.styleable.CalendarView_cv_selectTextSize, sp2px(14));//不可选中
        setCannotSelectTextSize(selectTextSizes);


        Drawable selectDayBackgrounds = a.getDrawable(R.styleable.CalendarView_cv_cannotselectDayBackground);//不可选中
        setCannotSelectDayBackground(selectDayBackgrounds);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int day = 1; day <= days; day++) {
            ...
            long time = DateUtils.getStringToDate(year + "年" + month + "月" + day + "日") + 72000000L;//晚上八点之前可点
            // 判断 day 是否在选择日期内
            if (mSelectDate == null || mSelectDate.size() == 0 ||
                    !mSelectDate.contains(getFormatDate(year, month - 1, day))) {//TODO

                long third = chooseArea * 86400L * 1000L;
                if ((startLillis + third) < time || startLillis > time) {//不可选择范围
                    drawBackground(canvas, mconnotSelectDayBackground, column, row);
                    drawText(canvas, dayStr, mconnotSelectTextColor, mconnotSelectTextSize, x, y);
                } else {//选择范围内
                ...
                drawText(canvas, dayStr, mconnotSelectTextColor, mSelectTextSize, x, y);
            }
        }
    }
    ...
    private void onClick(int day) {
        ...
        if (mIsChangeDateStatus) {
            ...
            if (mSelectDate != null && mSelectDate.contains(date)) {
                ...
                long time = DateUtils.getStringToDate(year + "年" + (month + 1) + "月" + day + "日") + 72000000L;
                long third = chooseArea * 86400L * 1000L;
                if ((startLillis + third) < time || startLillis > time) {
                } else {
     ...
     * @param textColor 文字颜色 {@link ColorInt}.
     */
    public void setCannotSelectTextColor(@ColorInt int textColor) {
        this.mconnotSelectTextColor = textColor;
    }
     ...
     * @param textSize 文字大小 (sp).
     */
    public void setCannotSelectTextSize(float textSize) {
        this.mconnotSelectTextSize = textSize;
    }
     ...
     * @param background 背景 drawable.
     */
    public void setCannotSelectDayBackground(Drawable background) {
        if (background != null && mconnotSelectDayBackground != background) {
            this.mconnotSelectDayBackground = background;
            setCompoundDrawablesWithIntrinsicBounds(mconnotSelectDayBackground);
        }
    }
     ...
     * 设置可选择范围的起始位置
     */
    public void setStartLillis(long startLillis) {
        this.startLillis = startLillis;
        invalidate();
    }
